...
def plot_convergence(result: BFGResult, save_path: Optional[str] = None, show: bool = True):
    ...
    conditional = any(c.get('metadata', {}).get('summary_semantics') ==
                      'evaluated-model conditional diagnostics' for c in result.checkpoints)
    ax.set_ylabel(r"Checkpoint $\log Z$", fontsize=11)
    ax.set_title('Observed-mass checkpoints; final point reconstructed' if conditional
                 else 'BFG Denominator Convergence', fontsize=12, fontweight='bold')
